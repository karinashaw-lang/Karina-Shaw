...
import { getRecentLiveChatMessages, postLiveChatMessage, type LiveChatMessageDto } from "@/lib/actions/live-chat";

const POLL_INTERVAL_MS = 3000;

export default function LiveChat({ liveStreamId }: { liveStreamId: string }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      const latest = await getRecentLiveChatMessages(liveStreamId);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setDraft("");
    setSending(true);
    await postLiveChatMessage(liveStreamId, body);
    const latest = await getRecentLiveChatMessages(liveStreamId);
    setMessages(latest);
    setSending(false);
  }
